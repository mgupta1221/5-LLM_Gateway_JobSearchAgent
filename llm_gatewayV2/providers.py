...
# Per-provider HTTP timeout in seconds. If an LLM takes longer than this the
# request is aborted and the router retries with the next provider in order.
...
class OpenAICompatProvider(BaseProvider):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        try:
            async with httpx.AsyncClient(timeout=PROVIDER_TIMEOUT) as c:
            ...
            raise ProviderError(
                f"{self.name} timed out after {PROVIDER_TIMEOUT}s",
    ...
    async def stream(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        try:
            ...
            raise ProviderError(
                f"{self.name} stream timed out after {PROVIDER_TIMEOUT}s",
                status=408, retryable=True,
            )
...
class GeminiProvider(BaseProvider):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        try:
            ...
            raise ProviderError(
                f"gemini timed out after {PROVIDER_TIMEOUT}s",
                status=408, retryable=True,
            )
...
# ────────────────────────────────────────────────────────────────────────────
# Anthropic (Claude) — uses its own wire format, not OpenAI-compatible
# ────────────────────────────────────────────────────────────────────────────

def _anthropic_supports_thinking(model: str) -> bool:
    m = (model or "").lower()
    return (
        "claude-3-7" in m or "claude-3.7" in m
        or ("claude" in m and any(x in m for x in ("opus-4", "sonnet-4", "haiku-4")))
    )
...
class AnthropicProvider(BaseProvider):
    ...
    def __init__(self, api_key, model, base_url="https://api.anthropic.com"):
        super().__init__(api_key, model, base_url.rstrip("/"))

    def _headers(self, extra_betas=None):
        headers = {
    ...
    def _build_system(self, system_blocks):
        system_text, blocks, has_cache = _flatten_system(system_blocks)
        if not system_text:
            return None
        if not has_cache:
            return system_text
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        # Inject JSON schema into system prompt — Anthropic has no native response_format
        if response_format and not tools:
            ...
            if rf.get("schema"):
                ...
                if isinstance(body.get("system"), str):
                    body["system"] = (body["system"] or "") + instruction
                elif isinstance(body.get("system"), list):
                    body["system"].append({"type": "text", "text": instruction})
                else:
        ...
        if reasoning and reasoning != "off" and _anthropic_supports_thinking(m):
            budget = {"low": 2000, "medium": 8000, "high": 16000}.get(reasoning, 4000)
        ...
        try:
            async with httpx.AsyncClient(timeout=PROVIDER_TIMEOUT) as c:
                r = await c.post(
                    ...
                    headers=self._headers(extra_betas),
                    json=body,
                )
                if r.status_code != 200:
                    raise ProviderError(
                ...
                usage = d.get("usage") or {}
                stop = d.get("stop_reason") or "end_turn"
                stop_norm = "tool_use" if tool_calls_out else (
                    "max_tokens" if stop == "max_tokens" else "end_turn"
                )
                return {
            ...
            raise ProviderError(
                f"anthropic timed out after {PROVIDER_TIMEOUT}s",
                status=408, retryable=True,
            )
...
def model_capabilities(provider_name: str, model: str, default_caps: dict) -> dict:
    ...
    if provider_name == "anthropic":
        caps["reasoning"] = _anthropic_supports_thinking(model)
